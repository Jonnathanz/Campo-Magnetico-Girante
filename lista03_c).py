...
'''
    ...
    Descrição:  Simulação do campo magnético girante de um motor de indução com
                alimentação trifásica, simétrica, e com falha a Fase C
'''
...
def main():
    ...
    plt.title('Campo girante com alimentação trifásica simétrica e falha em uma Fase')

    def update(t):
        t *= 0.05                                                   # Passo do tempo t para 0.05, variando de 0 até 6.25 conforme os frames
        ### Equações da FEM:
        # Fm = 1, w = 1; Falha em Fc
        Fa = np.array([np.cos(t), 0])
        Fb = np.cos(t - 2*np.pi/3)*np.array([np.cos(-2*np.pi/3), np.sin(-2*np.pi/3)])
        Ft = Fa + Fb
        ###
    ...
    plt.legend()
    anim.save('Lista_03_c.gif', dpi=80, writer='imagemagick')
    plt.show()
